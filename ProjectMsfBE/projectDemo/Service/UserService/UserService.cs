...
namespace projectDemo.Service.UserService
{
    public class UserService : IUserService
    {
        ...
        public UserService(IOrderRepository order,IEventRepository eventrp,IUserRoleRepository userRole, IUnitOfWork uow, IUserReposiotry userReposiotry, IMapper mapper, IRoleRepository roleRepository, IPemisstionRepository petRepository, IUserLoginRepository userLoginRepository, IRolePermissionRepository rolePermission)
        {
        ...
        public async Task<ApiResponse<UserProfile>> GetListEventByUserIDCreate(Guid guid)
        {
            try
            {
                ...
                if (status != 200)
                {
                    return ApiResponse<UserProfile>.FailResponse(Entity.Enum.EnumStatusCode.EVENTNOTFOUD, messger);
                ...
                return ApiResponse<UserProfile>.SuccessResponse(Entity.Enum.EnumStatusCode.SUCCESS, response);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
                return ApiResponse<UserProfile>.FailResponse(Entity.Enum.EnumStatusCode.SUCCESS, "thích làm test lỗi không");
         ...
         async Task<ApiResponse<string>> IUserService.Delete(Guid id)
        {
           var user = await _userReposiotry.GetUserByid(id);
            if(user ==null)
            {
                return ApiResponse<string>.FailResponse(Entity.Enum.EnumStatusCode.NOT_FOUND, "không tìm thấy user");
            }
        ...
        async Task<PageResponse<UserResponse>> IUserService.GetAll(UserQuery param)
        {
            ...
            if(param.PageSize < 10)
                param.PageSize = 10;
            var (users, total) = await _userReposiotry.GetAll(param.PageIndex, param.PageSize,param.Keyword,param.Role);
        ...
        //update
        async Task<ApiResponse<UserResponse>> IUserService.Update(Guid id, UserUpdateRequest request)
        {
            if (request == null)
            {
                return ApiResponse<UserResponse>.FailResponse(
                    Entity.Enum.EnumStatusCode.BAD_REQUEST, "Request is null");
            }
            var user = await _userReposiotry.GetUserByid(id);
            if(user == null)
            {
                return ApiResponse<UserResponse>.FailResponse(Entity.Enum.EnumStatusCode.NOT_FOUND, "KHông tìm thấy User");
            }
            ...
            if (!string.IsNullOrWhiteSpace(request.RoleName))
            {
                ...
                if (role == null)
                {
                    return ApiResponse<UserResponse>.FailResponse(Entity.Enum.EnumStatusCode.NOT_FOUND, "Role không tồn tại");
                }
            ...
            return ApiResponse<UserResponse>.SuccessResponse(Entity.Enum.EnumStatusCode.SUCCESS,userReponse);
