...
using projectDemo.Service.TickService;
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            builder.Services.AddAuthorization(options =>
            {
                ...
                options.AddPolicy(
                    "user.create",
                    policy => policy.RequireClaim("permission", "user.create")
                );

                options.AddPolicy(
                    "user.update",
                    policy => policy.RequireClaim("permission", "user.update")
                );

                options.AddPolicy(
                    "user.delete",
                    policy => policy.RequireClaim("permission", "user.delete")
                );

                options.AddPolicy(
                    "user.view",
                    policy => policy.RequireClaim("permission", "user.view")
                );

                //role
                options.AddPolicy(
                    "role.create",
                    policy => policy.RequireClaim("permission", "role.create")
                );

                options.AddPolicy(
                    "role.update",
                    policy => policy.RequireClaim("permission", "role.update")
                );

                options.AddPolicy(
                    "role.delete",
                    policy => policy.RequireClaim("permission", "role.delete")
                );

                options.AddPolicy(
                    "role.view",
                    policy => policy.RequireClaim("permission", "role.view")
                );

                //event
                options.AddPolicy(
                    "event.create",
                    policy => policy.RequireClaim("permission", "event.create")
                );

                options.AddPolicy(
                    "event.update",
                    policy => policy.RequireClaim("permission", "event.update")
                );

                options.AddPolicy(
                    "event.delete",
                    policy => policy.RequireClaim("permission", "event.delete")
                );

                options.AddPolicy(
                    "event.view",
                    policy => policy.RequireClaim("permission", "event.view")
                );

                options.AddPolicy(
                    "event.getTotalTickbyid",
                    policy => policy.RequireClaim("permission", "event.getTotalTickbyid")
                );

                options.AddPolicy(
                    "event.getTotalTickByUser",
